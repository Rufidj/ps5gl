...
/* Legacy single/dual-channel formats and depth internal format - accepted by
 * glTexImage2D the same way any other internalformat is (ignored, see its
 * own comment: only real RGBA8 data with a real, non-NULL upload is ever
 * actually converted). Pulled in by vendor/sdl-gpu's GL3/GL4 backend, which
 * still supports these for older content even on modern contexts. */
#define GL_LUMINANCE       0x1909
#define GL_LUMINANCE_ALPHA 0x190A
#define GL_ALPHA           0x1906
#define GL_DEPTH_COMPONENT16 0x81A5
#define GL_TEXTURE_INTERNAL_FORMAT 0x1003
/* glGetString query for the shading-language version string this backend
 * effectively targets (see PS5GL's own "no GLSL compiler" note at the top of
 * this file) - answered as a real, if slightly fictional, version string
 * rather than left undeclared, since vendor/sdl-gpu queries it to decide
 * which of its own shader source variants to request (a decision this
 * backend does not act on anyway - see ps5gl_program_use_precompiled). */
...
/* Debug-only: the ps5gpu_target_create() index a texture id was redirected
 * to by glFramebufferTexture2D, or -1 if it was never attached to a
 * framebuffer. For reaching into ps5gpu_debug_read_target_pixel() (see
 * ps5gpu.h) from calling code without duplicating this library's own
 * TexParams bookkeeping. */
...
/* No-op: real immediate-mode drawing, which this backend has never had any
 * of (see GL_TRIANGLES-only note on glDrawArrays/glDrawElements). Declared
 * only because vendor/sdl-gpu's renderer_GL_common.inl has one call site
 * gated behind "the driver's GL_VENDOR string contains Intel" (a decades-old
 * workaround for a specific Intel driver bug) - since PS5GL's own
 * glGetString(GL_VENDOR) always answers "PS5GL" (see ps5gl.c), that
 * condition is never true here and this is never actually invoked, only
 * compiled. */
...
/* No-op: there is only ever one colour output (see PS5GPU_TARGET_USER0's own
 * single-attachment model - no multiple render targets), so which buffer to
 * draw/read is not a real choice here. Declared so the standard depth-only
 * FBO idiom (glDrawBuffer(GL_NONE); glReadBuffer(GL_NONE);) compiles. */
#define GL_NONE 0
