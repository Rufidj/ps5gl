#!/bin/bash
# Builds the PS5GL milestone-2 test into out/<TITLE_ID>/, ready to copy to the console.
set -e
here=$(cd "$(dirname "$0")" && pwd)
out=$here/out
TITLE_ID=${TITLE_ID:-PPSA00092}
TITLE_NAME=${TITLE_NAME:-PS5GL test}
...
echo ">>> compile"
$CC -c -O1 -Wall -Wextra -I"$here/include" -I"$here/backend" -o "$out/main.o" "$here/test/main.c"
$CC -c -O1 -Wall -Wextra -I"$here/include" -I"$here/backend" -o "$out/ps5gl.o" "$here/src/ps5gl.c"
$CC -c -O1 -Wall -Wextra -I"$here/backend" -o "$out/ps5gpu.o" "$here/backend/ps5gpu.c"
